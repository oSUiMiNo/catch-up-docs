...
### 画面の向きを宣言していない理由
仕様書のWeb App Manifestの例には `"orientation": "any"` がありますが、外しています。この宣言があると、アプリの指定が端末の自動回転の設定より優先され、**自動回転をオフにしていても倒すだけで回ってしまう**ためです。他のアプリのように、回転をオフにしているあいだは回らず、必要なときだけ画面隅に出る回転ボタンで切り替えられる状態にしました。

宣言を外してもどの向きも許可される点は変わりません。決めるのが利用者になるだけで、10.6の「画面回転に対応」は満たしています。Android側も同じ理由で `twa-manifest.json` の `orientation` を `default`（向きを指定しない）にしています。
