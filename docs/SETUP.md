...
**Step 2** 文書リポジトリの**名前**を入れます。GitHubのオーナー名は公開URLから自動で埋まっているので、通常はそのままで構いません。

ブランチ名と文書一覧ファイルの場所は「詳細設定」の中にあります。既定値のままで動くので、開く必要はありません。

> リポジトリ名だけ入力が必要なのは、この名前を公開資産へ一切置かない方針のためです（AC-002）。設定ファイルへ焼き込むと、公開URLを見た第三者にどんな非公開リポジトリを持っているかが分かってしまいます。入力した名前は端末の中で暗号化して保存され、外へは出ません。
